...
## ⚠️ 当前状态：v0.3.0，核心功能已实现但未经真实账号验证

**已经能跑、有测试覆盖的部分**：

- 四种设备（郊狼 / 负鼠 / 爪印 / 灵猫）统一连接入口，一个按钮全覆盖，设备层已支持传输层注入
  （为安卓 Tauri BLE 铺路）
...
- 设置面板（provider 选择 + 密钥/模型/音色/语速 + 场景 + 权限 + 安全上限）与通话面板
...
- **未拿真实 API Key 做过端到端联调**——`session.update` 的具体字段形状、鉴权用的 WebSocket
  ...
  需要跟着真实报错调整的点（已确认 `api.x.ai/v1/tts/voices` 浏览器可直连、CORS 放行，但用假 key
  测试收到 400，真实鉴权格式仍需核实）
- 安卓壳还没搭（设备层已经做了传输注入的准备工作）
