...
在旧 DG-Agent 的「设置 → 数据」中导出 ZIP，然后在 0xNuller 的「设置 → 数据 → 导入」中
选择该文件。旧版单个 JSON、多会话 JSON 和新版 ZIP 都可以直接导入；同 ID 会话以导入文件为准。
导入失败不会清空现有记录。登录后会话会同步到账户，但模型密钥、设备连接和权限不会导入。
