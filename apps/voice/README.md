...
- 郊狼和负鼠沿用 DG-Lab transport 的单实例 client：Voice 同一时间各最多连接一台。选择同类型
  第二台设备会被拒绝，必须先断开旧设备，避免既有授权或排队命令迁移到新硬件。断开重连会生成
  新的本地 opaque 身份；设备名称不作为身份，也不会把同名设备合并。
- 通用设备运行时支持多个设备和多个振动功能。模型看到的是每个实例独立的 opaque `deviceId` 与
  `featureId`，每次工具调用必须精确指定一对 ID；名称不进入模型状态，命令不会广播或 fan-out。
- 输出命令在进入本地队列前会重新检查连接身份、权限结果和当前模块租约。身份或租约在确认期间
  发生变化时，本次增加输出会失败关闭；停止与全局紧急停止不受普通输出权限或租约阻断。
