 ...
 * so the confirmation experience is identical across both apps — same four
 * grant scopes, same "advanced" fold protecting the wide grants, same
 * Escape-to-deny behaviour.
 */
        ...
        <div className="mt-3 border-t border-[var(--surface-border)] pt-2">
          ...
          {advancedOpen && (
            <div className="mt-2 grid grid-cols-2 gap-2">
              <Button variant="secondary" className="rounded-[10px] text-[12px]" onClick={onAllowTimed}>
                允许 5 分钟
              </Button>
              <Button variant="secondary" className="rounded-[10px] text-[12px]" onClick={onAllowSession}>
                允许本会话
              </Button>
            </div>
          )}
