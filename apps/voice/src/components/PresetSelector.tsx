...
import { BUILTIN_PROMPT_PRESETS, type SavedPromptPreset } from '@voice/lib/prompts';
 ...
 */
export function PresetSelector({ settings, updateSettings }: PresetSelectorProps) {
  ...
  const hiddenIds = settings.hiddenBuiltinPresetIds;
  const visibleBuiltins = BUILTIN_PROMPT_PRESETS.filter((p) => !hiddenIds.includes(p.id));

  function selectPreset(id: string) {
    updateSettings((prev) => ({ ...prev, promptPresetId: id }));
  }

  function hideBuiltin(id: string) {
    updateSettings((prev) => {
      const nextHidden = [...prev.hiddenBuiltinPresetIds, id];
      let nextSelected = prev.promptPresetId;
      if (prev.promptPresetId === id) {
        const remaining = BUILTIN_PROMPT_PRESETS.filter((p) => !nextHidden.includes(p.id));
        nextSelected = remaining[0]?.id ?? prev.savedPromptPresets[0]?.id ?? prev.promptPresetId;
      }
      return { ...prev, hiddenBuiltinPresetIds: nextHidden, promptPresetId: nextSelected };
    });
  }

  function restoreBuiltins() {
    updateSettings((prev) => ({ ...prev, hiddenBuiltinPresetIds: [] }));
  }
  ...
  function saveEdit() {
    ...
    updateSettings((prev) => ({
      ...prev,
      savedPromptPresets: prev.savedPromptPresets.map((p) =>
        p.id === editingId ? { ...p, name: editName.trim(), icon: editIcon, prompt: editPrompt } : p,
      ),
    }));
    setEditingId(null);
  }
  ...
  function deletePreset(id: string) {
    updateSettings((prev) => ({
      ...prev,
      savedPromptPresets: prev.savedPromptPresets.filter((p) => p.id !== id),
      promptPresetId: prev.promptPresetId === id ? (BUILTIN_PROMPT_PRESETS[0]?.id ?? id) : prev.promptPresetId,
    }));
  }
  ...
  function confirmCreate() {
    ...
    updateSettings((prev) => ({
      ...prev,
      promptPresetId: id,
      savedPromptPresets: [...prev.savedPromptPresets, { id, name: newName.trim(), icon: newIcon, prompt: newPrompt }],
    }));
    setCreating(false);
  }
  ...
  function importFromMarket(item: MarketItem) {
    ...
    updateSettings((prev) => {
      if (prev.savedPromptPresets.some((p) => p.id === id)) {
        return { ...prev, promptPresetId: id };
      }
      return {
        ...prev,
        promptPresetId: id,
        savedPromptPresets: [...prev.savedPromptPresets, { id, name: item.name, icon: item.icon || DEFAULT_CUSTOM_ICON, prompt }],
      };
    });
  }
    ...
    <>
      <section className="settings-row-card space-y-2">
        <div className="flex items-center justify-between">
          ...
          {hiddenIds.length > 0 && (
            <Button variant="ghost" size="sm" className="h-7 gap-1 text-xs text-[var(--text-faint)]" onClick={restoreBuiltins}>
        ...
        <div className="space-y-1.5">
          {visibleBuiltins.map((preset) => {
            const active = settings.promptPresetId === preset.id;
            return (
              <div
                key={preset.id}
                className={cn(
                  'group flex w-full min-w-0 items-center gap-2 rounded-[10px] px-3 py-2.5 transition-colors',
                  active ? 'bg-[var(--accent-soft)] ring-1 ring-[var(--accent)]' : 'hover:bg-[var(--bg-soft)]',
                )}
              >
                <button type="button" className="flex min-w-0 flex-1 items-center gap-3 text-left" onClick={() => selectPreset(preset.id)}>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-0.5 truncate text-[12px] text-[var(--text-faint)]">{preset.description}</div>
                  </div>
                </button>
                <Button
                  variant="ghost"
          ...
          {visibleBuiltins.length === 0 && (
            <div className="py-4 text-center text-sm text-[var(--text-faint)]">所有内置场景已隐藏，点击右上角"恢复默认"找回</div>
          )}
        </div>
      </section>
      ...
      <section className="settings-row-card space-y-2">
        ...
        {settings.savedPromptPresets.length === 0 && !creating && (
          <div className="py-4 text-center text-sm text-[var(--text-faint)]">还没有自定义场景，点击下方按钮创建</div>
        )}

        <div className="space-y-1.5">
          {settings.savedPromptPresets.map((preset) => {
            if (editingId === preset.id) {
              return (
                <div key={preset.id} className="space-y-2 rounded-[12px] border border-[var(--accent)] bg-[var(--bg-strong)] p-3">
                  <div className="flex gap-2">
                    <EmojiPicker value={editIcon} onChange={setEditIcon} />
                    <Input value={editName} onChange={(e) => setEditName(e.target.value)} placeholder="场景名称" className="text-sm" />
                  </div>
                  <Textarea value={editPrompt} onChange={(e) => setEditPrompt(e.target.value)} rows={4} placeholder="描述 AI 的人设和互动风格…" className="text-sm" />
                  <div className="flex gap-2">
        ...
        {creating ? (
          <div className="space-y-2 rounded-[12px] border border-[var(--surface-border)] bg-[var(--bg-strong)] p-3">
            <div className="flex gap-2">
              ...
              <Input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="场景名称" className="text-sm" autoFocus />
            </div>
            <div className="flex items-center justify-between">
              <span className="text-xs text-[var(--text-faint)]">人设 / 互动风格</span>
              <Button variant="ghost" size="sm" className="h-6 gap-1 text-xs text-[var(--text-faint)] hover:text-[var(--accent)]" onClick={() => setNewPrompt(SCENE_TEMPLATE)}>
            ...
            />
            <div className="flex gap-2">
              <Button size="sm" onClick={confirmCreate} disabled={!newName.trim()}>创建</Button>
      ...
      <MarketImportDialog open={marketOpen} onOpenChange={setMarketOpen} type="scenario" onImport={importFromMarket} />
    </>
  );
}
