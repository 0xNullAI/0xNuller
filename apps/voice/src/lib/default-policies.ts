...
import type { OpossumPolicyRule, PolicyRule } from './policy-engine.js';
...
export interface DefaultPolicyOptions {
  ...
  // Extra caps that apply only to `burst` commands, narrower than the
  // per-channel strength cap. 0 (default) = disabled.
...
export function createDefaultOpossumPolicyRules(
  ...
  return [
    ...
    },
    {
      ...
      // ceiling the caller can never exceed regardless of how many small
      // vibrate_adjust steps stack up.
