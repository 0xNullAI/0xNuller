...
export function useWaveforms() {
  ...
  useEffect(() => {
    localStorage.setItem(HIDDEN_KEY, JSON.stringify(hiddenBuiltinIds));
  }, [hiddenBuiltinIds]);
  ...
  const addRemoteWaveform = useCallback((waveform: WaveformDefinition) => {
    setCustomWaveforms((prev) =>
      prev.some((w) => w.id === waveform.id) ? prev : [...prev, { ...waveform, custom: true }],
    );
  }, []);
  ...
  const addMarketWaveform = useCallback((item: MarketItem) => {
    ...
    setCustomWaveforms((prev) =>
      prev.some((w) => w.id === waveform.id) ? prev : [...prev, waveform],
    );
  }, []);
