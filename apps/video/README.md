...
- 郊狼、负鼠与 NativeBridge 提供的通用振动功能进入同一个共享输出目标列表；该列表与 Control 复用 `UnifiedOutputTarget` / `OutputTargetPicker`，不再先选设备家族再选目标。每个条目保留物理目标或运行时功能的完整身份，Video 仍只精确授权当前选中的一个目标。
...
- Web 可同时连接并在统一列表中逐一选择、断开多个郊狼；负鼠和每个通用振动功能也在同一列表中选择。切换或断开当前授权目标时，必须先中止推理并确认 stop，再改变选择或断开传输。无法证明多设备身份的原生传输仅暴露一个带临时 ID 的郊狼连接。
