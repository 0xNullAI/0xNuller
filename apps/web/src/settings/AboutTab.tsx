    ...
    <div className="flex max-w-lg flex-col gap-5">
      ...
      <section className="border-t border-[var(--surface-border)] pt-4">
        ...
        <textarea value={message} onChange={(e) => setMessage(e.target.value)} maxLength={4000}
          placeholder="请描述问题或建议" className="mt-3 min-h-28 w-full rounded-[var(--radius-ctl)] border border-[var(--surface-border)] bg-[var(--bg-soft)] p-3 text-sm" />
        <input value={contact} onChange={(e) => setContact(e.target.value)} maxLength={254}
          placeholder="联系方式（可选）" className="mt-2 min-h-10 w-full rounded-[var(--radius-ctl)] border border-[var(--surface-border)] bg-[var(--bg-soft)] px-3 text-sm" />
        <div className="mt-2 flex items-center gap-3">
          <button type="button" disabled={sending || !message.trim()} onClick={async () => {
            setSending(true); setStatus(null);
            try { await submitFeedback({ message, contact }); setMessage(''); setContact(''); setStatus('已提交，感谢反馈。'); }
            catch (e) { setStatus(e instanceof Error ? e.message : '提交失败，请稍后重试'); }
            finally { setSending(false); }
          }} className="inline-flex min-h-10 items-center gap-2 rounded-[var(--radius-ctl)] bg-[var(--accent)] px-3 text-sm text-white disabled:opacity-50"><Send className="h-4 w-4" />{sending ? '提交中…' : '提交反馈'}</button>
          {status && <span role="status" className="text-xs text-[var(--text-soft)]">{status}</span>}
        </div>
      </section>
