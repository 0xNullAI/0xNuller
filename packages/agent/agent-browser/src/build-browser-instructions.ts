...
export interface BrowserInstructionsInput {
  ...
  /** Current civet-edging state; disconnected devices are omitted from model context. */
...
export function createBuildBrowserInstructions(settings: BrowserInstructionSettings) {
  return (input: BrowserInstructionsInput): string => {
    ...
    const blocks = [
      ...
      buildBehaviorRulesBlock(hasConnectedDevice(input)),
      ...
      input.isFirstIteration ? buildFirstIterationStrategyBlock(hasConnectedDevice(input)) : '',
      !input.isFirstIteration ? buildFollowUpIterationBlock(hasConnectedDevice(input)) : '',
    ];
...
function buildDeviceBlock(input: BrowserInstructionsInput): string {
  ...
  if (input.opossumState?.connected) {
    lines.push(
  ...
  if (input.pawPrintsState?.connected) sensorLabels.push('爪印（按键 / 姿态传感器）');
  if (input.civetEdgingState?.connected) sensorLabels.push('灵猫（压力传感器）');
...
function buildDeviceMappingBlock(input: BrowserInstructionsInput): string {
  ...
  if (input.session.deviceState.connected) {
    lines.push(
      ...
      '4. 结束电击 / 解除：必须调用 shock_stop 停止对应通道，不允许只在文字里写"已经关掉了"而设备仍在运行。',
  ...
  if (input.pawPrintsState?.connected || input.civetEdgingState?.connected) {
    const sensors = [
      input.pawPrintsState?.connected ? '爪印' : '',
    ...
    lines.push(
      `连接的传感器（${sensors.join(' / ')}）只会通过 [内部提醒] 系统消息把事件推送给你，你不能主动查询或调用工具触发读数。`,
      outputs.length > 0
        ? `收到事件后按剧情自行判断是否需要用当前连接的输出设备（${outputs.join(' / ')}）做出响应，不必每次都操作设备。`
        : '收到事件后只根据读数回应，不要声称执行了任何输出动作。',
    );
  }
...
function buildBehaviorRulesBlock(hasDevices: boolean): string {
  const lines = [
    '[行为规则]',
    '1. 工具报错、被拒绝、权限未通过时，要如实告知用户，不要假装成功，也不要立刻重复同一个工具调用。',
  ];
  if (hasDevices) {
    lines.push(
      '2. 需要操作设备时，先调用对应工具，再根据工具结果回复用户。',
      '3. 回复设备状态时，只引用 [当前设备状态] 和本回合工具返回的事实，不要臆测。',
...
function buildFirstIterationStrategyBlock(hasDevices: boolean): string {
  if (!hasDevices) {
    return [
      '[本回合策略 - 仅本回合首次响应生效]',
      '直接回应用户当前的问题；不要为了确认未提供的信息而额外调用工具。',
    ].join('\n');
  }
  return [
    '[本回合策略 - 仅本回合首次响应生效]',
    '1. 如果用户明确要求某个设备动作，只执行最小必要的一步，不要自己连做 start + 多次 adjust_strength。',
    '2. 如果用户只是聊天、问状态、问建议，直接文字回复即可；不要为了"确认一下"额外调用工具。',
    '3. 做完一步动作后就停下，基于真实结果回复，并询问用户是否满意或是否继续。',
  ].join('\n');
}

function buildFollowUpIterationBlock(hasDevices: boolean): string {
  if (!hasDevices) {
    return '[后续迭代提醒]\n优先收口回答已经获得的结果，避免重复调用工具。';
  }
  return [
    '[后续迭代提醒]',
...
function buildDeviceStatusBlock(
  ...
  if (input.opossumState?.connected) {
    ...
    const oConnection = `已连接${o.deviceName ? `（${o.deviceName}）` : ''}`;
  ...
  if (input.pawPrintsState?.connected) {
    lines.push(
  ...
  if (input.civetEdgingState?.connected) {
    lines.push(
      ...buildSensorStatusLines(
        '灵猫',
...
function buildSensorStatusLines(
  ...
  const connection = `已连接${state.deviceName ? `（${state.deviceName}）` : ''}`;
