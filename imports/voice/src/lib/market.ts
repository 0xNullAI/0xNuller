// DG-Market 社区市场客户端：拉取他人上传的场景，供「从市场导入」使用。
// 部署 DG-Market 后，把下面的 VITE_MARKET_BASE_URL 设为你的 Worker 地址，
// 或直接改这里的兜底常量。Ported verbatim from DG-Agent's apps/web/src/lib/market.ts
// (DG-Voice only imports scenarios, not waveforms — design_wave already covers that).
