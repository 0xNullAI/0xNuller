...
## ⚠️ 当前状态：v0.6.0，核心功能已实现但未经真实账号验证

**已经能跑、有测试覆盖的部分**：

- **安卓版**：`apps/tauri-android`（Tauri + tauri-plugin-blec 多设备 fork），签名 APK 已发布到
  GitHub Release。整套 UI 和安全链跟网页版共用同一份 `../../src`，安卓特有代码只有传输注入那一个文件；
  麦克风权限、锁屏即停都已就位。同样只做郊狼 + 负鼠，不接传感器
