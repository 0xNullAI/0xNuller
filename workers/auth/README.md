...
- 路由：`/api/auth/*`
- D1：账户、会话、资料、联系人、私聊索引、同步内容、角色和用户反馈
- R2：用户资料照片
...
用户反馈会先写入 D1，再尽力转发到维护邮箱；邮件临时失败不会丢失已保存内容。公开反馈接口按
来源哈希限制为每小时最多 5 次，不保存原始 IP。

管理面板将付费充值和 Credit 赠送分开记账。充值要求外部支付流水；赠送由系统生成唯一编号并要求
填写原因，不计入充值收入。两类人工入账都记录操作管理员并受单笔和每日上限约束。
